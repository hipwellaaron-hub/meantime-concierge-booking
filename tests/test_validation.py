...
def test_thursday_finish_after_close_warns():
    ...
    assert "midweek_finish_after_close" in codes(warnings)
...
def test_wednesday_finish_after_close_warns():
    # Wednesday 2026-08-05, finishing at 9:30pm -- same rule as Thursday.
    warnings = validate_trading_hours(dt.date(2026, 8, 5), dt.time(21, 30))
    assert "midweek_finish_after_close" in codes(warnings)


def test_wednesday_finish_at_close_is_clean():
    warnings = validate_trading_hours(dt.date(2026, 8, 5), dt.time(21, 0))
    assert codes(warnings) == set()


def test_friday_finish_after_9pm_not_subject_to_midweek_rule():
    # Same late finish, but it's a Friday -- rule is Wednesday/Thursday only.
    warnings = validate_trading_hours(dt.date(2026, 8, 7), dt.time(23, 0))
    assert "midweek_finish_after_close" not in codes(warnings)
