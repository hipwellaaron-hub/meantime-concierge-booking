...
class Booking(Base):
    ...
    # The identity of ONE form submission, generated by the browser when
    # the form is rendered and sent with the POST. A retry after a lost
    # response, or two clicks on Submit, carry the same id and resolve to
    ...
    # Every browser and server attempt to send this enquiry's conversion,
    # per platform and channel -- see app/models/conversion_dispatch.py.
    conversion_dispatches = relationship(
        "ConversionDispatch", back_populates="booking", cascade="all, delete-orphan", order_by="ConversionDispatch.created_at"
    )
